...
@Controller
@RequestMapping("/sys/tjreport")
public class TjReportController extends ForestryBaseController<TjReport>{
	...
	public void getTjReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isNotBlank(requirementNo)) {
			tjReport.set$like_requirementNo(requirementNo);
		}
		
		tjReport.setFirstResult(firstResult);
		tjReport.setMaxResults(maxResults);
		Map<String, String> sortedCondition = new HashMap<String, String>();
		sortedCondition.put(sortedObject, sortedValue);
		tjReport.setSortedConditions(sortedCondition);
		QueryResult<TjReport> queryResult = tjReportService.doPaginationQuery(tjReport);
		List<TjReport> tjReportList = tjReportService.getReportList(queryResult.getResultList());
		ListView<TjReport> tjReportListView = new ListView<TjReport>();
		tjReportListView.setData(tjReportList);
		tjReportListView.setTotalRecord(queryResult.getTotalCount());
		writeJSON(response, tjReportListView);	
	}
	
	@RequestMapping(value = "/saveReport", method = { RequestMethod.POST, RequestMethod.GET })
	public void savetjReport(TjReport entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
		ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
		if (null == entity.getId()) {
			parameter.setSuccess(false);
		} else {
			tjReportService.update(entity);
	...
	public void importDesignFile(@RequestParam("id") Integer id,@RequestParam(value = "importedFile", required = false) MultipartFile file, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!file.isEmpty()) {
				try {
					...
					tjReportService.updateReportLoc(id, fileName);
	...
	public void generateReport(TjReport entity) throws IOException {
		...
		dataMap.put("tjsgUnit", bs.getTjsgUnit());  
		dataMap.put("openDate", sdfDate.format(entity.getOpenDate()));  
		dataMap.put("closeDate", dateStr);  
		dataMap.put("towerType", entity.getTowerType());  
		dataMap.put("baseType", entity.getBaseType()); 
		dataMap.put("stubLength", entity.getStubLength()); 
		dataMap.put("diam", entity.getDiam()); 
		dataMap.put("steelBrand", entity.getSteelBrand()); 
		dataMap.put("concreteNo", entity.getConcreteNo());  
		dataMap.put("roomType", entity.getRoomType()); 
		dataMap.put("size", entity.getSize()); 
			
		dataMap.put("year", arr[0]);
		dataMap.put("month", arr[1]);
		dataMap.put("day", arr[2]);
		dataMap.put("tjsgUnit2", bs.getTjsgUnit()); 
		...
		String fileName = filePath.getAbsolutePath() + "/" + entity.getRequirementNo() + "-TjReport.doc";
		doc.createDoc(dataMap, new File(basePath ), "TjReportTemplate.ftl", fileName);
	}
}
