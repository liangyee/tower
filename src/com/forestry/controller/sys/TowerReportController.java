...
@Controller
@RequestMapping("/sys/towerreport")
public class TowerReportController extends ForestryBaseController<TowerReport>{
	...
	public void getReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		QueryResult<TowerReport> queryResult = towerReportService.doPaginationQuery(report);
		List<TowerReport> reportList = towerReportService.getReportList(queryResult.getResultList());
		ListView<TowerReport> towerReportListView = new ListView<TowerReport>();
		towerReportListView.setData(reportList);
		towerReportListView.setTotalRecord(queryResult.getTotalCount());
		writeJSON(response, towerReportListView);	
	}
	
	@RequestMapping(value = "/saveReport", method = { RequestMethod.POST, RequestMethod.GET })
	public void saveReport(TowerReport entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
		ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
		if (null == entity.getId()) {
			parameter.setSuccess(false);
		} else {
			towerReportService.update(entity);
	...
	public void importDesignFile(@RequestParam("id") Integer id,@RequestParam(value = "importedFile", required = false) MultipartFile file, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!file.isEmpty()) {
				try {
					...
					towerReportService.updateReportLoc(id, fileName);
	...
	public void generateReport(TowerReport entity) throws IOException {
		...
		dataMap.put("towerUnit", bs.getTowerUnit());  
		dataMap.put("openDate", sdfDate.format(entity.getOpenDate()));  
		dataMap.put("closeDate", dateStr);  
		dataMap.put("type", entity.getType());  
		dataMap.put("height", entity.getHeight()); 
		dataMap.put("weight", entity.getWeight()); 
		dataMap.put("installType", entity.getInstallType()); 
		dataMap.put("platNum", entity.getPlatNum()); 
		dataMap.put("platType", entity.getPlatType());
		dataMap.put("year", arr[0]);
		dataMap.put("month", arr[1]);
		dataMap.put("day", arr[2]);
		dataMap.put("towerUnit2",  bs.getTowerUnit());   
		...
		String fileName = filePath.getAbsolutePath() + "/" + entity.getRequirementNo() + "-TowerReport.doc";
		doc.createDoc(dataMap, new File(basePath ), "TowerReportTemplate.ftl", fileName);
	}
}
